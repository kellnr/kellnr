...
This folder contains the **Playwright Test** based smoke/integration tests that will gradually replace the existing Lua-based tests in `tests/`.

During the migration:
- `tests/` stays as-is and continues to run in CI.
- `tests2/` is the new home for Playwright-based tests.
- We port **one test at a time** to keep debugging straightforward.
...
- Cargo configured to publish to the local Kellnr registry used by these tests (same requirement as the Lua tests)
...
- `TESTS2_DEBUG=1` (enables extra debug logging in the helper utilities)
...
- `tests2/test-results/`
- `tests2/playwright-report/`

## Adding new tests / porting strategy

When porting a Lua test from `tests/test-*/`:
1. Create a corresponding `*.spec.ts` in `tests2/src/`.
2. Reuse shared orchestration helpers from `tests2/src/testUtils.ts`.
3. Keep the behavior equivalent to the Lua version (same env vars, same publish steps, same readiness checks).
4. Prefer attaching logs on failure (and optionally also on success while the migration is ongoing).
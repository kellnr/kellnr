...
/**
 * Central docker/testcontainers helper library for tests2.
 *
 * Goals:
...
export type StartContainerOptions = {
   ...
   *
   * This restores the Lua-test-style setup where Kellnr always runs on localhost:8000 and
...
function tests2Root(): string {
  // Playwright runs with cwd tests2/ (see existing tests2 code). Keep robust anyway.
 ...
 *
 * NOTE: This function assumes it's called from within tests2 (process.cwd() == tests2 root).
 */
...
export async function startContainer(
  ...
  if (options.ports) {
    ...
    //
    // This allows a stable localhost:8000 setup (like the Lua tests).

#!/usr/bin/env python3
"""
Generate a "Keep a Changelog"-style JSON document from release notes.
...
Inputs (env vars; all optional but recommended):
  ...
  - RELEASE_BODY                Release body / release notes (markdown text)
...
- This is a heuristic mapper. It works best when release notes are either:
  - GitHub auto-generated notes ("* x by @y in https://...") containing
    conventional-commit-like subjects, OR
  - release-plz / Keep a Changelog markdown sections ("### Added", ...), OR
...
def _extract_items_from_kac_sections(body: str) -> List[Tuple[str, str]]:
    """Parse Keep a Changelog style sections.

    Supports release-plz output like:
    ...
    Returns a list of (section, item) pairs.

    Notes:
    - Only the known KAC sections are recognized (case-insensitive).
    - Lines must be bullet/numbered list items under a recognized section.
    """
    current: str | None = None

    # map common headings to canonical KAC section names
    section_map = {s.lower(): s for s in KAC_SECTIONS}
    ...
    for raw in body.splitlines():
        ...
        # match headings like "### Added" (any count of '#')
...
def build_payload(
    *,
    ...
    # 1) Prefer explicit Keep a Changelog sections if present (release-plz format).
    kac_items = _extract_items_from_kac_sections(body)
    if kac_items:
        for section, item in kac_items:
            payload["keep_a_changelog"][section].append(item)
        return payload

    # 2) Fallback: GitHub autogenerated notes / conventional-commit bullets.

...
test.describe("docs generation smoke test", () => {
  // Fixed-port setup: Kellnr runs on stable localhost:8000, and crate-local `.cargo/config.toml`
  // can remain static.
  restrictToSingleWorkerBecauseFixedPorts();
  ...
  test("generates docs for published crate", async ({}, testInfo) => {
    ...
    // Expected docs path on the host (same structure as the legacy smoke test)

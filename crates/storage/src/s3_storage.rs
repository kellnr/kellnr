...
impl TryFrom<(&str, &Settings)> for S3Storage {
    ...
    fn try_from((bucket, settings): (&str, &Settings)) -> Result<Self, Self::Error> {
        ...
        if let Some(endpoint) = settings.s3.endpoint.as_deref().filter(|s| !s.trim().is_empty()) {
            s3 = s3.with_endpoint(endpoint);
        }
        if let Some(region) = settings.s3.region.as_deref().filter(|s| !s.trim().is_empty()) {
            s3 = s3.with_region(region);
        }
        if let Some(access_key) = settings.s3.access_key.as_deref().filter(|s| !s.trim().is_empty())
        {
            s3 = s3.with_access_key_id(access_key);
        }
        if let Some(secret_key) = settings.s3.secret_key.as_deref().filter(|s| !s.trim().is_empty())
        {
